...
@objc @IBDesignable open class CircleProgressView: UIView {
    ...
    override open func draw(_ rect: CGRect) {
        ...
        let changedRadius = radius - trackWidth * 0.5
        if roundedCap {
            let capCenter = CGPoint(x: center.x + changedRadius * cos(endAngle), y: center.y + changedRadius * sin(endAngle))
        ...
        if roundedCap {
            let capCenter = CGPoint(x: center.x + changedRadius * cos(startAngle), y: center.y + changedRadius * sin(startAngle))
        ...
        // center Drawing
        let centerPathWidth = innerRect.width - (2 * trackWidth)
        let centerPathHeight = innerRect.height - (2 * trackWidth)
        let centerPath = UIBezierPath(ovalIn: CGRect(x: innerRect.minX + trackWidth, y: innerRect.minY + trackWidth, width: centerPathWidth, height: centerPathHeight))
